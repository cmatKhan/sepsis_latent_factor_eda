...
# All samples/features from GSE13904 (config/GSE13904_config.yml) --
...
# lowercase `symbol` column rather than control_type / gene_symbol). See
# R/preprocessing/gse13904_day1_preprocessing.R /
# gse13904_day3_preprocessing.R for the same pattern applied to a single
# timepoint's samples.
framework_run <- exists("tmp_dir", inherits = FALSE)

if (!framework_run) {
    expression_path <- "~/projects/hf_sepsis_collection/GSE13904/expression.parquet"
    sample_metadata_path <- "~/projects/hf_sepsis_collection/GSE13904/sample_metadata.parquet"
    feature_metadata_path <- "~/projects/hf_sepsis_collection/GSE13904/feature_metadata.parquet"
}
